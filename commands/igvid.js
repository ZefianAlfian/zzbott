...
exports.run = async (bot, message, args, from) => {
    ...
    reply(mess.wait)
    febycans = await fetchJson(`https://mhankbarbar.tech/api/ig?url=${args[0]}&apiKey=${apiMank}`, {method: 'get'})
    if (febycans.error) return reply(febycans.error)
    divara = await getBuffer(febycans.result)
    bot.sendMessage(from, divara, video, {quoted: message, caption: mess.success})
